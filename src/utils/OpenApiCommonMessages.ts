...
export function readProtoMessage(pbf: PBF, end?: number) {
  return pbf.readFields(
    readProtoMessageField,
    { payloadType: 0, payload: undefined, clientMsgId: "" },
    end
  );
}
...
export function writeProtoErrorRes(obj: ProtoErrorRes, pbf: PBF) {
  if (obj.payloadType != null && obj.payloadType !== 50)
...
function readProtoHeartbeatEventField(
  tag: number,
  obj: ProtoHeartbeatEvent,
  pbf: PBF
) {
  if (tag === 1) obj.payloadType = pbf.readVarint();
}
export function writeProtoHeartbeatEvent(obj: ProtoHeartbeatEvent, pbf: PBF) {
  if (obj.payloadType != null && obj.payloadType !== 51)
    pbf.writeVarintField(1, obj.payloadType);
}
